...
echo  "$($PSQL "SELECT COUNT(*) FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE (winner_goals > 2)")" 
...
echo  "$($PSQL "SELECT DISTINCT(teams.name) FROM teams FULL JOIN games ON (teams.team_id = games.winner_id) OR (teams.team_id = games.opponent_id) WHERE (games.round = 'Eighth-Final' AND games.year = 2014) ORDER BY name")"
